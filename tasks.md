...
- [x] **TU.6 Docs** — README (camera click-to-start, shipped dist), CLAUDE.md (dist convention).
...
Scan card polish 2026-06-12: voucherPayload() now carries entry_title ((string) cast of the reservation's augmented entry title; '## Entry deleted ##' fallback comes free from Resrv's emptyEntry()) and rate (getRateLabel(), null when neither the reservation nor any child has a rate_id — "if applicable" per user request). Scan.vue card lists Entry + Rate above Reference; Mark as used / Scan another promoted to variant="primary" (ghost read as plain text at the door). New rate-attached lookup test + entry_title/rate assertions on the happy path; README + CHANGELOG; fresh cp:build dist. Pest 76/223 green; pint clean.
Dashboard widget 2026-06-15: new src/Widgets/Vouchers.php (handle `vouchers`) — a v6-native VueComponent widget (resources/js/components/widgets/Vouchers.vue, registered via Statamic.component('resrv-vouchers-widget') in cp.js) with Scan / View-all-vouchers buttons + the card title linking to the list. component() returns null unless User::current()?->can('use resrv vouchers') (mirrors core Collection widget — hides even when the dashboard config omits `can`). Registered through VouchersProvider's new protected $widgets. Site owners opt in via the cp.php widgets array (`'type' => 'vouchers'`). New DashboardWidgetTest (4 cases: handle registration, permitted render with scan/list URLs, title override, null for unpermitted). README CP-usage + CHANGELOG; fresh cp:build dist. Pest 80/230 green; pint clean.
Mark-used toast 2026-06-22 (user request): Scan.vue fires a native Statamic success toast ("QR successfully marked as used") after a successful mark-used PATCH. New resources/js/composables/useToast.js mirrors Resrv's $toast wrapper (success/error). The toast lives on the 200 path only — markUsed() returns the voucher payload exclusively when this request performed the transition; every failure, including a lost concurrency race (conditional UPDATE matches 0 rows → InvalidVoucherTransitionException → 422), lands in catch and surfaces the existing inline error, so there's no false "success" when another scanner won the race. JS-only change; fresh cp:build dist; no PHP/pest change (suite unchanged at 80/230).
